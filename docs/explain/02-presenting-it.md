...
4. **How we know.** One command, `python3 scripts/check_all.py`, runs all 103 automated
   tests (226 backend, 174 frontend), the type-check, build and lint, then starts the real
   servers and tests them live. It prints 24 pass/fail lines, all passing. GitHub runs the
...
| 7 | Run `python3 scripts/check_all.py` | "One command runs all 400 tests, the build and lint, then starts the real app and tests it live: 24 lines, all passing." |
...
`python3 scripts/check_all.py` runs the 226 backend and 174 frontend tests, the
type-check, build and lint, then starts the real backend and page and sends real
messages through them — 24 pass/fail lines. We also checked it by hand in a browser
