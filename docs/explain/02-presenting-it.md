...
4. **How we know.** One command, `python3 scripts/check_all.py`, runs all 103 automated
   tests (283 backend, 191 frontend), the type-check, build and lint, then starts the real
   servers and tests them live. It prints 32 pass/fail lines, all passing. GitHub runs the
...
| 7 | Run `python3 scripts/check_all.py` | "One command runs all 474 tests, the build and lint, then starts the real app and tests it live: 32 lines, all passing." |
...
`python3 scripts/check_all.py` runs the 283 backend and 191 frontend tests, the
type-check, build and lint, then starts the real backend and page and sends real
messages through them — 32 pass/fail lines. We also checked it by hand in a browser
