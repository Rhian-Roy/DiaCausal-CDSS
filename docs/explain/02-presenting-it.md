...
4. **How we know.** One command, `python3 scripts/check_all.py`, runs all 103 automated
   tests (382 backend, 238 frontend), the type-check, build and lint, then starts the real
   servers and tests them live. It prints 41 pass/fail lines, all passing. GitHub runs the
...
| 7 | Run `python3 scripts/check_all.py` | "One command runs all 620 tests, the build and lint, then starts the real app and tests it live: 41 lines, all passing." |
...
`python3 scripts/check_all.py` runs the 382 backend and 238 frontend tests, the
type-check, build and lint, then starts the real backend and page and sends real
messages through them — 41 pass/fail lines. We also checked it by hand in a browser
